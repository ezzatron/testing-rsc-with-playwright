...
export default function Pokemans({ chunkNumber }: Props) {
  const [result] = useQuery({
    query: ListPokemans,
    variables: { offset: chunkNumber * PAGE_SIZE, limit: PAGE_SIZE },
  });

  const { data, fetching, error } = result;

  if (fetching) return <LoadSkeleton />;
  if (error || !data) {
    return <p>Oh no... {error?.message ?? "there's no data"}</p>;
  }
